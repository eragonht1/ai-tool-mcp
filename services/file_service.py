                ...
                if not is_valid:
                    ...
                    try:
                        result = await self._run_sync(
                            self.file_option.create_directory, path
                        )
...
返回每个目录的删除结果"""
            ...
            for path in paths:
                ...
                if not is_valid:
                    ...
                    try:
                        result = await self._run_sync(
                            self.file_option.delete_directory, path, force
                        )
                        if result.get("success", True):
                            results.append(
                                {
...
返回每个复制操作的结果"""
            ...
            for op in ops:
                ...
                try:
                    result = await self._run_sync(
                        self.file_option.copy_file, source, dest
                    )
                    if result.get("success", True):
                        results.append(
                            {
...
返回每个移动操作的结果"""
            ...
            for op in ops:
                ...
                try:
                    result = await self._run_sync(
                        self.file_option.move_file, source, dest
                    )
                    if result.get("success", True):
                        results.append(
                            {
...
返回每个文件的删除结果"""
            ...
            for path in paths:
                ...
                if not is_valid:
                    ...
                    try:
                        result = await self._run_sync(
                            self.file_option.delete_file, path
                        )
                        if result.get("success", True):
                            results.append(
                                {
        ...
        async def chmod(
            operations: Annotated[
                ...
                Field(
                    description="""文件权限修改操作，支持单个或批量修改
...
返回每个权限修改操作的结果"""
            ...
            for op in ops:
                ...
                try:
                    result = await self._run_sync(
                        self.file_option.change_file_permissions, file_path, mode
                    )
                    if result.get("success", True):
                        results.append(
                            {
