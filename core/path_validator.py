...
class AbsolutePathValidator:
    ...
    def validate_path(path: str, allow_none: bool = False) -> Tuple[bool, str]:
        ...
        if platform.system() == "Windows":
            ...
            if not (len(path) >= 3 and path[1] == ':' and path[2] in ['\\', '/']):
                return False, f"Windows绝对路径格式错误，正确格式: C:\\path\\to\\file，当前输入: {path}"
        else:
            # Unix/Linux路径检查
    ...
    @staticmethod
    def validate_multiple_paths(paths: Union[str, List[str]], allow_none: bool = False) -> Tuple[bool, str, List[str]]:
        """
        验证多个路径
